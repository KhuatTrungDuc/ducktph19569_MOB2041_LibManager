...
import ducktph19569.fpoly.ducktph19569_mob2041_libmanager.SPINERADAPTER.LoaiSachSpiner;
...
public class S_Adapter extends RecyclerView.Adapter<S_Adapter.SachHoder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull @NotNull SachHoder holder, int position) {
        ...
        if (holder.tv_tacgia.getText().toString().contains("1")){
            holder.tv_tacgia.setTextColor(Color.RED);
            holder.tv_gias.setTextColor(Color.RED);
            holder.tv_mls.setTextColor(Color.RED);
            holder.tv_tens.setTextColor(Color.RED);
        }else if(holder.tv_tacgia.getText().toString().contains("2")){
            holder.tv_tacgia.setTextColor(Color.RED);
            holder.tv_gias.setTextColor(Color.RED);
            holder.tv_mls.setTextColor(Color.RED);
            holder.tv_tens.setTextColor(Color.RED);
        }else if (holder.tv_tacgia.getText().toString().contains("3")){
            holder.tv_tacgia.setTextColor(Color.RED);
            holder.tv_gias.setTextColor(Color.RED);
            holder.tv_mls.setTextColor(Color.RED);
            holder.tv_tens.setTextColor(Color.RED);
        }else if(holder.tv_tacgia.getText().toString().contains("10")){
            holder.tv_tacgia.setTextColor(Color.GREEN);
            holder.tv_gias.setTextColor(Color.GREEN);
            holder.tv_mls.setTextColor(Color.GREEN);
            holder.tv_tens.setTextColor(Color.GREEN);
        }else if (holder.tv_tacgia.getText().toString().contains("11"))
            holder.tv_tacgia.setTextColor(Color.GREEN);
        holder.tv_mls.setTextColor(Color.GREEN);
        holder.tv_gias.setTextColor(Color.GREEN);
        holder.tv_mls.setTextColor(Color.GREEN);
        ...
        if (sach == null) {
            ...
            holder.tv_tacgia.setText("Số lượng: " + sach.getTacgia());
            Locale locale = new Locale("nv", "VN");
//            NumberFormat numberFormat = NumberFormat.getCurrencyInstance(locale);
//            String tien = numberFormat.format(sach.getGias());
            holder.tv_gias.setText("Giá: " + sach.getGias());
        }
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (strSearch.isEmpty()) {
                    ...
                    for (Sach sach : mlistOld) {
                        if (sach.getTacgia().contains(strSearch)) {
                            listtv.add(sach);
                        }
                        ;
                    }
...
//                    for (Sach thanhVien : mlistOld) {
//                        if (thanhVien.getGias().contains(strSearch)) {
